...
const LEARN_CARDS = [
  ...
  {
    title: 'Why the viewer?',
    body: `
      <p>Reading a dozen reports of a hundred pages each, side by side,
      does not scale. The viewer supports the comparisons directly:
      isolate one disclosure category, select a passage, and inspect its
      nearest neighbors across every competitor at once:</p>
      <svg class="proj-demo" id="tour-demo" width="480" height="230"
           viewBox="0 0 480 230"></svg>
      <p>The same machinery underlies what follows: per-category
      comparisons across an industry, and \u2014 as further report years are
      ingested \u2014 how each firm\u2019s disclosure moves over time.</p>`,
  },
  {
...
function renderLearnCard() {
  ...
  if (document.getElementById('tour-demo')) startTourDemo();
  learnCard.focus();
}

// card 2: a stylized tour of the workflow - the corpus, a category
// isolated, a passage selected, its neighbors spotlit, the passage read.
function startTourDemo() {
  const svg = document.getElementById('tour-demo');
  // fixed pseudo-random scatter; category 1 (water-blue) is the story
  let seed = 41;
  const rand = () => (seed = (seed * 1103515245 + 12345) & 0x7fffffff) / 0x7fffffff;
  const CAT_COLORS = ['#8ca252', '#17becf', '#c47b3d'];
  const PTS = [];
  for (let i = 0; i < 42; i++) {
    const cat = i % 3;
    const cx = [140, 250, 350][cat] + (rand() - 0.5) * 150;
    const cy = 105 + (rand() - 0.5) * 130;
    PTS.push([cx, cy, cat]);
  }
  const SELi = PTS.findIndex(p => p[2] === 1 && p[0] > 220 && p[0] < 285);
  ...
  const PHASES = ['the corpus \u2014 three categories of disclosure',
                  ...
                  'read the passage; every point carries its source'];
  svg.innerHTML = `<g id="td-pts"></g><g id="td-extra"></g>
    <text id="td-cap" x="240" y="222" font-size="11.5" text-anchor="middle"
      fill="#6b6b76"></text>`;
  const ptsG = svg.querySelector('#td-pts');
  const extra = svg.querySelector('#td-extra');
  const cap = svg.querySelector('#td-cap');
  function frame(ts) {
    if (!document.getElementById('tour-demo')) return;
    const phase = Math.floor(ts / 1800) % PHASES.length;
    let html = '', ex = '';
    for (let i = 0; i < PTS.length; i++) {
      const [x, y, cat] = PTS[i];
      const dimmed = phase >= 1 && cat !== 1;
      html += `<circle cx="${x}" cy="${y}" r="4.5"
        fill="${CAT_COLORS[cat]}" opacity="${dimmed ? 0.12 : 0.8}"/>`;
    }
    if (phase >= 2) {
      const [sx, sy] = PTS[SELi];
      ex += `<circle cx="${sx}" cy="${sy}" r="8" fill="none"
              stroke="#5b4b8a" stroke-width="2"/>
             <circle cx="${sx}" cy="${sy}" r="4.5" fill="#5b4b8a"/>`;
    }
    if (phase >= 3) {
      const [sx, sy] = PTS[SELi];
      for (const j of NBRS) {
        const [nx, ny] = PTS[j];
        ex += `<line x1="${sx}" y1="${sy}" x2="${nx}" y2="${ny}"
                stroke="#a45a1e" stroke-width="1" stroke-dasharray="3 3"/>
               <circle cx="${nx}" cy="${ny}" r="7" fill="none"
                stroke="#a45a1e" stroke-width="1.6"/>`;
      }
    }
    if (phase >= 4) {
      // a small reading pane sketch beside the selection
      const [sx, sy] = PTS[SELi];
      const px = Math.min(sx + 24, 330), py = Math.max(sy - 46, 12);
      ex += `<rect x="${px}" y="${py}" width="130" height="64" rx="7"
              fill="#fffdf9" stroke="#5b4b8a" stroke-width="1.1"/>
             <rect x="${px + 10}" y="${py + 11}" width="72" height="5" rx="2.5"
              fill="#17becf"/>`;
      for (let l = 0; l < 3; l++) {
        ex += `<rect x="${px + 10}" y="${py + 24 + l * 11}"
                width="${110 - l * 16}" height="4.5" rx="2"
                fill="#ddd3c2"/>`;
      }
    }
    ptsG.innerHTML = html;
    extra.innerHTML = ex;
    cap.textContent = PHASES[phase];
...
// Auto-open whenever the reader arrives from outside the site (an
// external link, a shared URL, a fresh tab). Navigation between SUE's
// own pages does not re-trigger it; the Guide button reopens any time.
try {
  const sameSite = document.referrer
    && new URL(document.referrer).origin === location.origin;
  if (!sameSite) openLearn();
} catch (e) { openLearn(); }
